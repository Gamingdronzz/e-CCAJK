...
public class PanAdhaarUploadActivity extends AppCompatActivity {
    ...
    private void uploadAdhaarOrPan() {
        ...
        Date date = new Date();
        PanAdhaar panAdhaar = new PanAdhaar(code, number.getText().toString(), fileChosed, Preferences.getInstance().getPrefState(this));

        dbref.child(code).setValue(panAdhaar).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(PanAdhaarUploadActivity.this, "Request sent for upload", Toast.LENGTH_SHORT).show();
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(PanAdhaarUploadActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
